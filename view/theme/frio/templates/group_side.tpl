<div class="widget" id="group-sidebar">
	...
	<h3>{{$title}}</h3>
	{{if ! $newgroup}}
	<a class="pull-right" href="{{$grouppage}}" title="{{$editgroupstext}}" data-toggle="tooltip">
	<i class="faded-icon fa fa-pencil" aria-hidden="true"></i>
	</a>
	{{else}}
	<a class="pull-right" id="sidebar-new-group" onclick="javascript:$('#group-new-form').fadeIn('fast');" title="{{$createtext}}" data-toggle="tooltip">
			...
			{{foreach $groups as $group}}
				<li role="menuitem" class="sidebar-group-li group-{{$group.id}} {{if $group.selected}}selected{{/if}}">
					...
					{{if $group.edit}}
						...
						<a id="edit-sidebar-group-element-{{$group.id}}" class="group-edit-tool pull-right" href="{{$group.edit.href}}" title="{{$edittext}}" data-toggle="tooltip">
							<i class="faded-icon fa fa-pencil" aria-hidden="true"></i>
						</a>
					{{/if}}
